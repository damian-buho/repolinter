...
describe(
  'cli',
  () => {
    const repolinterPath: string =
      process.platform === 'win32'
        ? path.resolve('dist/cli.js')
        : path.resolve('dist/cli.js')
    const selfPath: string = path.resolve('tests/cli')
    ...
    it('fixes a problem with dryRun disabled', async () => {
      ...
      assert.strictEqual(isFilePresent, true)
    })
    ...
    it("doesn't make any changes with dryRun enabled", async () => {
      ...
      assert.strictEqual(isFilePresent, false)
    })
    ...
    it('runs repolinter from the CLI using a config file', async () => {
      ...
      const selfBinPath: string = path.resolve(selfPath, 'bin')
    ...
    it('runs repolinter using a remote ruleset', async () => {
      ...
      const port: number = server.getHttpPort()
      server.on({
        method: 'GET',
      ...
      let expected: string
      let actual: ExecResult
      let actual2: ExecResult
      let actual3: ExecResult
      try {
        expected = stripAnsi(
          repolinter.defaultFormatter.formatOutput(
    ...
    it('runs repolinter using invalid encoded ruleset', async () => {
      const encodedRuleset: string = 'ewogICAgIiRzY2hlf'
      const [actual, actual2] = await Promise.all([
        execAsync(`"${repolinterPath}" lint -c ${encodedRuleset}`),
        execAsync(`"${repolinterPath}" lint --rulesetEncoded ${encodedRuleset}`)
      ])
    ...
    it('runs repolinter with ruleset file path too long', async () => {
      ...
      const encodedRuleset: string = 'a'.repeat(4097)
      const [actual, actual2] = await Promise.all([
        execAsync(`"${repolinterPath}" lint -c ${encodedRuleset}`),
        execAsync(`"${repolinterPath}" lint --rulesetEncoded ${encodedRuleset}`)
      ])
    ...
    it('should handle encoded rulesets with encoded extends', async () => {
      ...
      assert.ok(
        actual.out
          .trim()
          .includes(`test-file-exists: ignored because level is "off"`)
      )
      assert.ok(
        actual2.out
          .trim()
