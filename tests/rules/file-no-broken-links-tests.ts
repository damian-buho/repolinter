...
import fileNoBrokenLinks from '../../src/rules/file-no-broken-links.js'
...
function createMockServer(responses: MockResponses): http.Server {
  ...
  () => {
    describe('files_no_broken_links', () => {
      ...
      it('returns true if no links are present in markdown', async () => {
        ...
        assert.strictEqual(actual.targets[0].path, 'no_link.md')
      })
      ...
      it('returns true if a valid link is present in a markdown file', async () => {
        ...
        const temporaryFile = path.join(targetDirectory, '_test_valid_link.md')
        fs.writeFileSync(
          temporaryFile,
      ...
      it('ignores section links in markdown', async () => {
        ...
        assert.strictEqual(actual.targets.length, 1)
        assert.strictEqual(actual.targets[0].passed, true)
        assert.strictEqual(actual.targets[0].path, 'section_link.md')
      })

      it('ignores .rst files (no longer rendered)', async () => {
        const ruleopts = {
        ...
        assert.strictEqual(
          ...
          'Ignored due to unknown file format.'
        )
      })
